...
public class TestJdbcDriver2 {
  ...
  public void testCreateTableAsExternal() throws SQLException {
    Connection con = getConnection(testDbName + ";hiveCreateAsExternal=true");
    Statement stmt = con.createStatement();
    ResultSet res = stmt.executeQuery("set hive.create.as.external");
    assertTrue("ResultSet is empty", res.next());
    assertEquals("hive.create.as.exteranl=true", res.getObject(1));
    stmt.close();
    con.close();
  }
